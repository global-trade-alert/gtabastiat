...
b_squad_formation = function(squad.level=.7
                             ){
  ...
  load("content/0 core/Squad statistics & history.Rdata")
  if(length(setdiff(bt.squad$member.name,
    ...
    new.sq=bt.squad
    new.sq$squad.number=max(all.squads$squad.number)+1
  ...
  squad.result=merge(squad.result, unique(training[,c("bid", "evaluation")]), by="bid", all.x=T)

  squad.result=reshape(squad.result, idvar = c("bid", "evaluation"), timevar = "detective", direction="wide")
  rm(squad.predictions)

  # Estimating squad classifier
  squad.estimation=b_estimate_classifier(training.data=squad.result,
                                         training.id="bid",
  ...
  squad.classifier=squad.estimation$classifier
