...
b_create_model_variables <- function(bid=NULL,
  ...
  #### adding it back to tf
  tf=merge(tf, word.score, by="word", all.x = T)
  if("gini.normalised" %in% unique(c(my.vars, dtm.metric))){
    tf=merge(tf, gini[,c("word","gini.normalised")], by="word", all.x=T)
  }
  ...
  for(gg in c("gta.gini.all","gta.gini.source","gta.gini.title","gta.gini.description")){
    if(gg %in% unique(c(my.vars, dtm.metric))){
      ...
      tf=merge(tf, subset(gta.gini, gta.text==g.var)[,c("word","gta.gini.delta")], by="word", all.x=T)
      setnames(tf, "gta.gini.delta",gg)
    }
    print(gg)
  }

  if(sum(as.numeric((c("odds.relevant","odds.irrelevant", "odds.ratio") %in% unique(c(my.vars, dtm.metric)))))>0){
    tf=merge(tf, odds[,c("word","odds.relevant","odds.irrelevant", "odds.ratio" )], by="word", all.x=T)
  }
  ...
  if(length(intersect(c("gta.share.all","gta.share.source","gta.share.title", "gta.share.description"), unique(c(my.vars, dtm.metric))))>0){
    ...
    for(gs in intersect(c("gta.share.all","gta.share.source","gta.share.title", "gta.share.description"), unique(c(my.vars, dtm.metric)))){
      ...
      tf=merge(tf, subset(gta.words, text==g.var)[,c("word","gta.share.word")], by="word", all.x=T)
      setnames(tf, "gta.share.word",gs)
    }
  ...
  tf.agg=aggregate(score.delta ~ bid + evaluation, tf, sum)
  ...
  if(dtm.incl==T){
    ...
    nr.terms.xgb=terms.xgb
    nr.terms.svm=terms.svm

    eval(parse(text=paste("tf=tf[order(tf$",dtm.metric,", decreasing = T),]", sep="")))
    svm.words=unique(tf$word)[1:terms.svm]
    xgb.words=unique(tf$word)[1:terms.xgb]

    tf.dtm=count(subset(tf, word %in% svm.words), vars=c('bid', 'word'))
    ...
    tf.agg=merge(tf.agg, tf.dtm[,c("bid", xgb.words)], by="bid", all.x=T)
