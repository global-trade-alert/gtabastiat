...
bt_store_dpa_source = function(timeframe = 365*3,
  ...
  # Get status list
  url.status.list = dbGetQuery(con, "SELECT * FROM gta_url_status_list")
  ...
  if(nrow(missing.sources)>0){
    ...
    for(url in unique(missing.sources$source_url)){
      ...
      if(src.url!="NA"){
        ...
        # should only be done for sources where scrape has already been attempted and failed
        wayback.fail = F
        if(use.wayback){
          if((most.recent.attempt$is_success==0 & most.recent.attempt$check_status_id!=12)){
            ...
            #TODO what if a source page is referenced in several events w different dates because the page changed over time?
            ...
            # NOTTODO add the internet archive URL to gta_url_log and gta_measure_url... maybe as separate col?
            # I think if ALL the URLs are recorded anyway, this is redundant
        ...
        if(scrape.result$status %in% url.status.list$id[url.status.list$is_success==1]){
          ...
          for(sa.id in unique(subset(missing.sources, url == src.url)$measure_id)){
            ...
            # The lux_event_source.display_on_flag determines if the source url
            # should be shown on the frontend. There can always only be one
            # entry per event_id marked display_on_flag = 1.So technically, if
            # you add a new redirected url, you’d have to check if that original
            # url was marked display_on_flag = 1, and if so, mark the new one
            # display_on_flag = 1  while setting the original display_on_flag =
            # 0
            
            # OK. I would usually trust the user specified link of the
            # autogenerated one, since sometimes a 'redirect' flag can indicate
            # when a request redirects to a generic 'article not found' page
            # which is useless 09:31 yes I have a 404-catching logic, but
            # there's no standardised way of specifying if a page is 404, so
            # some will slip through 09:31 I will set the display_on_flag to 0
            # for the new entries in that case
            
            lux.event.source.sql = glue("INSERT INTO lux_event_source (event_id, source_id, display_on_flag)
                                        VALUES ({lux.event.id}, {lux.source.id.just.added}, 0)")
          ...
          cat(glue("updating lux_source_log with failed status #{scrape.result$status}: "))
          lux.source.log.sql = glue("UPDATE lux_source_log
                           SET last_check = CURRENT_TIMESTAMP, check_status_id = {scrape.result$status}
                           WHERE source_url = '{url}';")
